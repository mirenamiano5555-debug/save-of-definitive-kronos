...
BEGIN
  ...
  SELECT EXISTS (SELECT 1 FROM public.banned_emails WHERE email = NEW.email) INTO _is_banned;
  IF _is_banned THEN
    RAISE EXCEPTION 'This email address has been banned and cannot register.';
  END IF;
...
EXCEPTION WHEN OTHERS THEN
  IF SQLERRM = 'This email address has been banned and cannot register.' THEN
    RAISE;
  END IF;
